...
export const FETCH_RELATED_SUCCESS = 'FETCH_TRAILERS_SUCCESS';
...
function fetchRelatedSuccess(data) {
  //console.log(data)
  return {
...
function fetchRelatedFail(error) {
  return {
...
export function fetchRelatedList(id){
  const url_related = URL_DETAIL + id + URL_SIMILAR + API_KEY;
  return function(dispatch){
    dispatch(fetchRelated())
    return fetch(url_related)
      .then(response => response.json())
      .then(json => json.results)
      .then(data => dispatch(fetchRelatedSuccess(data)))
      .catch(error => dispatch(fetchRelatedFail(error)))
  }
}